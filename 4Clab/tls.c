...
void id_stringer(char* id)
{
	...
	if(SSL_write(cSSL, result, len) < 0)
	{
...
void print_temp(uint16_t value)
{
	...
	strcpy(result, time_buff);
	strcat(result, " ");
	strcat(result, num);
	//strcat(result, gcvt(temp, 3, num));
	strcat(result, "\n");
...
void* pthreader()
{
	...
	while(run_flag)
	{
		...
		do
		{
			ret = SSL_read(cSSL, buff, BUFF_SIZE);	
			if(ret < 0)
			{
...
int main(int argc, char* argv[]) 
{
    ...
    if(sslctx == NULL)
    {
    	fprintf(stderr, "ERROR: failed to initiate SSL_CTX\n");
    	exit(1);
    }
    ...
    if(cSSL == NULL)
    {
    	fprintf(stderr, "ERROR: failed to initiate SSL\n");
    	exit(1);
    }
    if(SSL_set_fd(cSSL, netfd) == 0)
    {
    	fprintf(stderr, "ERROR: failed to set SSL fd\n");
    	exit(1);
    }
